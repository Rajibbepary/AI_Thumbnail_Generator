        ...
        <>
            ...
            >
                ...
                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    ...
                    {
                        isLoggedIn ? <Link to={'/mygeneration'} className="hover:text-pink-500 transition">
                    Generations
                    </Link>
                    : <Link to={'#'} className="hover:text-pink-500 transition">
                        About
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                <Link onClick={() => setIsOpen(false)} to='/'>Home</Link>
                 <Link onClick={() => setIsOpen(false)} to={'/generate'}>
                    ...
                    {isLoggedIn ? <Link onClick={() => setIsOpen(false)} to={'/mygeneration'}>
                     Generations
                    ...
                    {isLoggedIn ? <button onClick={() => {setIsOpen(false); logout()}} >Logout</button> : <Link onClick={() => setIsOpen(false)} to={'/login'}>
                    Login
                    </Link>} 
