...
const navigate = useNavigate()
   ...
   useEffect(()=>{
    ...
    return (
        <>
            <SoftBackdrop/>
            <div className="mt-32 min-h-screen px-6 md:px-16 lg:24 xl:32">
                {/* HEADER */}
                <div className="mb-8">
                    ...
                    {thumbnails.map((thumb: IThumbnail)=>{
                        ...
                        return (
                           <div key={thumb._id} onClick={()=> navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                                {/* IMAGE */}
                                <div className={`relative rounded-t-2xl ${aspectClass} bg-black`}>
                                    {thumb.image_url ? (
                                ...
                                <div className="p-4 space-y-2">
                                    ...
                                    <p className="text-xs text-zinc-500">{new Date(thumb.color_scheme !).toDateString()}</p>
                                </div>
                                <div onClick={(e)=>e.stopPropagation()} className="absolute bottom-2 right-2 max-md:flex sm:hidden group-hover:flex gap-1.5">
                                    <TrashIcon
